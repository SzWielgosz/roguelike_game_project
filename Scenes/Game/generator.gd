extends Node2D
#
#var FirstRoom = preload("res://Scenes/Rooms/14x14/room_left_right.tscn")
#
		...
		#while queue.is_empty() == false:
			...
			#for key in door_directions.keys():
						...
						#var generated_room_rect = Rect2(
							...
							#- picked_room.get_node("RoomArea").get_child(0).shape.extents 
						...
						#else:
							#room.queue_free()
							#room_rects.erase(generated_room_rect)
#
		...
		#for room in $"../Rooms".get_children():
			...
			#for key in door_directions.keys():
					...
					#var generated_room_rect = Rect2(
						#room.position 
						#- room.get_node("RoomArea").get_child(0).shape.extents 
						#+ spawn_directions[key] - Vector2(length, length), 
						#Vector2(length, length) * tile_size
					#)
					#if not does_overlap(generated_room_rect):
						#print("Found blind doors")
